...
window.onload = function() {
    ...
    document.addEventListener("click", ev => {
        if (ev.target.classList.contains("user-btn")) {
            playerNameInput.value = ev.target.getAttribute("id");
...
function setupSocket(socket) {
    ...
    // Handle movement.
    socket.on("serverTellPlayerMove", function(
        userData,
        foodsList,
        massList,
        virusList
...
function drawPlayers(order) {
    ...
    for (var z = 0; z < order.length; z++) {
        ...
        for (var i = 0; i < points; i++) {
            ...
            if (typeof userCurrent.id == "undefined") {
                x = valueInRange(
                    ...
                    global.gameWidth - userCurrent.x + global.screenWidth / 2,
                    x
                );
                y = valueInRange(
                    -userCurrent.y + global.screenHeight / 2,
                    global.gameHeight - userCurrent.y + global.screenHeight / 2,
                    y
                );
            } else {
                x = valueInRange(
        ...
        if (global.toggleMassState === 0) {
            graph.drawImage(
                window[nameCell + "_img"],
                circle.x - fontSize * 3,
                circle.y - fontSize * 3,
                fontSize * 6,
                fontSize * 6
            );
            graph.strokeText(nameCell, circle.x, circle.y + fontSize * 2);
            graph.fillText(nameCell, circle.x, circle.y + fontSize * 2);
        } else {
...
function drawgrid() {
    ...
    for (
        var x = global.xoffset - player.x;
        x < global.screenWidth;
        x += global.screenHeight / 18
    ) {
        graph.moveTo(x, 0);
    ...
    for (
        var y = global.yoffset - player.y;
        y < global.screenHeight;
        y += global.screenHeight / 18
    ) {
        graph.moveTo(0, y);
...
function drawborder() {
    ...
    if (player.y <= global.screenHeight / 2) {
        ...
        graph.lineTo(
            global.gameWidth + global.screenWidth / 2 - player.x,
            global.screenHeight / 2 - player.y
        );
        graph.strokeStyle = global.lineColor;
        graph.stroke();
...
function resize() {
    ...
    player.screenWidth = c.width = global.screenWidth =
        global.playerType == "player" ? window.innerWidth : global.gameWidth;
    player.screenHeight = c.height = global.screenHeight =
        global.playerType == "player" ? window.innerHeight : global.gameHeight;
